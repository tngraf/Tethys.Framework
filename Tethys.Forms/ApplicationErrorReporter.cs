...
namespace Tethys.Forms
{
    ...
    public class ApplicationErrorReporter
    {
        ...
        public void HandleThreadException(object sender,
          ThreadExceptionEventArgs threadExceptionEventArgs)
        {
            var result = DialogResult.Cancel;
            try
            {
                result = ShowThreadExceptionDialog(
            ...
            catch
            {
                try
                {
                    MessageBox.Show(this.textSevereError, this.textSevereError,
        ...
        {
            var form
              = new ApplicationExceptionForm(ApplicationAssembly);
            form.ExceptionText = e.Message;
            form.StackText = string.Format(CultureInfo.CurrentCulture,
              "Application = {0}\r\nVersion = {1}\r\n" +
             "Path = {2}\r\n\r\n{3}:{4}\r\n{5}",
              Application.ProductName, Application.ProductVersion,
              Application.ExecutablePath, e.GetType(), e.Message, e.StackTrace);
