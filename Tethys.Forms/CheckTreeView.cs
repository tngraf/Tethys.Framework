...
namespace Tethys.Forms
{
    ...
    public partial class CheckTreeView : TreeView
    {
        ...
        public void SetCheck(CheckTreeNode node, CheckValue newValue)
        {
            ...
            {
                // raise event
                var ea = new TreeViewCancelEventArgs(SelectedNode,
            ...
            while (parent != null)
            {
                CheckValue valChildren = GetCheckChildren(parent);
                SetCheck(parent, valChildren);
        ...
        {
            // check node
            SetCheck(node, newValue);

            // check all children
            for (int i = 0; i < node.Nodes.Count; i++)
            {
                SetCheckValue((CheckTreeNode)node.Nodes[i], newValue);
            } // for
        } // SetCheckEx()
        ...
        public static CheckValue GetCheckChildren(TreeNode node)
        {
            ...
            CheckValue ret = ((CheckTreeNode)node.Nodes[0]).Checked;
            for (int i = 0; i < node.Nodes.Count; i++)
            {
                bool tristate = (ret != ((CheckTreeNode)node.Nodes[i]).Checked);
                if (tristate)
                {
        ...
        protected override void OnClick(EventArgs e)
        {
            ...
            var node = (CheckTreeNode)GetNodeAt(pt);
            if (IsHitCheckBox(node, pt))
            {
        ...
        private void HandleStateChange(CheckTreeNode node)
        {
            ...
            CheckValue val = GetCheck(node);
            val = GetNextCheckValue(val);
            if (this.triStateStyle)
            {
                SetCheckValue(node, val);
            }
            else
            {
        ...
        protected override void OnKeyDown(KeyEventArgs e)
        {
            ...
            {
                HandleStateChange((CheckTreeNode)SelectedNode);
            } // if
        } // OnKeyDown()
        ...
        protected bool IsHitCheckBox(TreeNode node, Point pt)
        {
            ...
            var rc = new RECT();
            rc.left = (int)node.Handle;
            IntPtr ok = Win32Api.SendMessage(this.Handle, Msg, 1, ref rc);
            if (ok != IntPtr.Zero)
            {
                if (pt.X > rc.left)
                {
