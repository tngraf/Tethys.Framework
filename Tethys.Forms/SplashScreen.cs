...
namespace Tethys.Forms
{
    ...
    public partial class SplashScreen : Form, IMessageFilter
    {
        ...
        private void InitializeSplashBitmap()
        {
            ...
            Stream stream = this.SourceAssembly.GetManifestResourceStream(this.SplashBitmapPath);
        ...
        private void PaintHandler(object sender, PaintEventArgs e)
        {
            ...
            int rightBorder = this.Size.Width - 3;
            int offset = 0;
            int posAppName = 130;
            int posDescLine1 = 168;
            int posVersion = 212;
            ...
            grfx.DrawString(str, fnt18B, Brushes.Black,
             rightBorder, posAppName, sf);

            str = this.TextProductDescription;
            grfx.DrawString(str, fnt12B, Brushes.Black,
              rightBorder, posDescLine1, sf);

            if (this.UseVersionInfo)
            {
                Version version = SourceAssembly.GetName().Version;
                str = VersionInfo.GetLevel(this.SourceAssembly, version);
                AssemblyReleaseModeAttribute releaseMode =
                  (AssemblyReleaseModeAttribute)Attribute.GetCustomAttribute(this.SourceAssembly,
                  typeof(AssemblyReleaseModeAttribute));
                if (releaseMode.ReleaseMode == ReleaseMode.Test)
                {
                    str += "\n";
            ...
            grfx.DrawString(str, fnt10B, Brushes.Black,
              rightBorder, posVersion, sf);
        } // PaintHandler()
