...
namespace Tethys.Forms
{
    ...
    public class RecentFileList
    {
        ...
        /// </remarks>
        public string GetDisplayName(string name, int index, string curDirectory)
        {
            ...
            var strDir = Path.GetDirectoryName(this.mruItemText[index]);
            ...
            {
                name = Extensions.AbbreviatePath(name, this.maxDisplayLength, true);
            } // if
        ...
        {
            var regkey = Registry.CurrentUser.OpenSubKey(this.sectionName);
            if (regkey == null)
            {
                // list does not exist
                // -> empty
            ...
            for (var mru = 0; mru < this.listSize; mru++)
            {
                var strEntry = this.entryFormat + (mru + 1);
                this.mruItemText[mru] = (string)regkey.GetValue(strEntry, string.Empty);
            } // for
        } // ReadList()
        ...
        public void UpdateMenu(ToolStripMenuItem menuItemStart)
        {
            ...
            var indexStart = parent.DropDownItems.IndexOf(menuItemStart);
            ...
            if (!this.initDone)
            {
                ...
                for (var mru = 0; mru < this.listSize; mru++)
                {
                    ...
                    parent.DropDownItems.Insert(index++, this.toolstripItems[mru]);
