// ---------------------------------------------------------------------------
// <copyright file="Form1.cs" company="Tethys">
...
namespace TgVersionInfoDemo
{
    ...
    public class Form1 : Form
    {
        ...
        private void Form1Load(object sender, EventArgs e)
        {
            ...
            var dayAttribute =
              (AssemblyDayAttribute)Attribute.GetCustomAttribute(assembly, typeof(AssemblyDayAttribute));
            Debug.Assert(dayAttribute != null, "dayAttribute must not be empty!");
            this.labelDay.Text = dayAttribute.Day.ToString(culture);

            var monthAttribute =
              (AssemblyMonthAttribute)Attribute.GetCustomAttribute(assembly, typeof(AssemblyMonthAttribute));
            Debug.Assert(monthAttribute != null, "monthAttribute must not be empty!");
            this.labelMonth.Text = monthAttribute.Month.ToString(culture);

            var yearAttribute =
              (AssemblyYearAttribute)Attribute.GetCustomAttribute(assembly, typeof(AssemblyYearAttribute));
            Debug.Assert(yearAttribute != null, "yearAttribute must not be empty!");
            this.labelYear.Text = yearAttribute.Year.ToString(culture);

            var firstYearAttribute =
              (AssemblyFirstYearAttribute)Attribute.GetCustomAttribute(assembly, typeof(AssemblyFirstYearAttribute));
            Debug.Assert(firstYearAttribute != null, "firstYearAttribute must not be empty!");
            this.labelFirstYear.Text = firstYearAttribute.FirstYear.ToString(culture);

            var releaseModeAttribute =
              (AssemblyReleaseModeAttribute)Attribute.GetCustomAttribute(assembly, typeof(AssemblyReleaseModeAttribute));
            Debug.Assert(releaseModeAttribute != null, "releaseModeAttribute must not be empty!");
            this.labelReleaseMode.Text = releaseModeAttribute.ReleaseMode.ToString();

            var version = assembly.GetName().Version;

            this.labelGetDateBuild.Text = VersionInfo.GetDateBuild(assembly);
            this.labelGetLevel.Text = VersionInfo.GetLevel(assembly, version);
            this.labelGetVersion.Text = VersionInfo.GetVersion(assembly, version, culture);
        }
    }
}
