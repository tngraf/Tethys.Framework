...
namespace RingBufferDemo
{
    ...
    public class MainForm : Form
    {
        ...
        {
            if (txtSizeNew.Text.Length > 0)
            {
                int n = int.Parse(txtSizeNew.Text);
                if (n > 0)
                {
                    rb = new RingBuffer(n);
                } // if
        ...
        private void UpdateStatus()
        {
            ...
            {
                txtSize.Text = rb.Size.ToString(CultureInfo.InvariantCulture);
                txtPosition.Text = rb.Position.ToString(CultureInfo.InvariantCulture);
                txtContents.Text = rb.GetData();
            } // if
        } // UpdateStatus()
        ...
        {
            if (txtDataAdd.Text.Length > 0)
            {
                rb.AddData(txtDataAdd.Text);
        ...
        private void BtnAddCharArryClick(object sender, EventArgs e)
        {
            if (txtDataAdd.Text.Length > 0)
            {
                char[] ca = txtDataAdd.Text.ToCharArray();
                rb.AddData(ca, ca.Length);
        ...
        {
            if (txtCharReq.Text.Length > 0)
            {
                int n = int.Parse(txtCharReq.Text);
                if (n > 0)
                {
                    txtDataReturn.Text = rb.GetData(n);
                } // if
            }
            else
            {
                txtDataReturn.Text = rb.GetData();
            } // if
        ...
        {
            if (txtConsume.Text.Length > 0)
            {
                int n = int.Parse(txtConsume.Text);
                if (n > 0)
                {
                    rb.Consume(n);
        ...
        {
            txtDataReturn.Text = rb.GetNextLine();
